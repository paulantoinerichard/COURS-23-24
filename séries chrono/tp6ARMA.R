...
n <- length(Rain)
temps <- 1:n
plot(temps, Rain, main=" ", xlab="Temps", ylab="", typ='l')

RegLin <- lm(Rain ~ temps)
...
#ARMA(0,0), on regarde la moyenne

# utiliser autoarima voir ce que ca donne
...
Volcano
LogVolcano=log(Volcano)

n=length(Volcano)
temps <- 1:n
plot(temps, LogVolcano, main=" ", xlab="Temps", ylab="", typ='l')

RegLin <- lm(LogVolcano ~ temps)
